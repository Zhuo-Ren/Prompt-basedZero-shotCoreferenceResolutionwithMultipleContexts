    ...
    def __eq__(self, other):
        for key in self.__dict__.keys():
            if torch.is_tensor(self.__dict__[key]):
                # 对于tensor，不同机器上训练的结果会有少许不同，不能严格判等。所以这里只要对应位置上的数字差别不大于0.0001就认为相等。
                diff = self.__dict__[key] - other.__dict__[key]
                if not sum(diff > 0.0001):
                    pass
                else:
                    return False
            else:
...
class EntityMention(Mention):
    ...
    def __init__(self, doc_id, sent_id, tokens_numbers,tokens,mention_str, head_text, head_lemma,
        ...
        """predicates里存放的是实体指称的谓词信息，key是一个元组，('谓词指称的文本表示'，'谓词指称的id')，value是论元类型（A0，A1, AM-TMP,AM-LOC）"""
