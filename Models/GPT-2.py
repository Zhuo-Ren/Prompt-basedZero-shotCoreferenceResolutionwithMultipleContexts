...
Version 1: GPT-2.ipynb的.py版本。
Version 2: 优化模型调用方式，为多模型统一调用做准备
后续整合到all.py中，此文件弃用
"""
...
from transformers import logging, GPT2TokenizerFast, GPT2Model, get_linear_schedule_with_warmup, pipeline, set_seed, GPT2LMHeadModel
import spacy
...
def gpt2_create(prefix):
    model = pipeline('text-generation', model='gpt2', return_full_text=False, prefix=prefix, device=torch.cuda.current_device())
    return model


def gpt2_pred(model, prompt):
    pred = model(prompt, max_new_tokens=1, num_return_sequences=1)[0]["generated_text"]
    return pred


def chatglm6b_create():
    return 1


def chatglm6b_pred(model, prompt):
    return 1
...
def annotate(data, templates, prefixes, prefixes_source, repeated_n, model_name):
    ...
    for i, template in enumerate(tqdm(templates, desc="模板", position=0, leave=False, ncols=60)):
        ...
        # 构建模型
        if model_name == "GPT2":
            model = gpt2_create(prefix)
        elif model_name == "ChatGLM-6B":
            model = chatglm6b_create()
        # 遍历每个文件
        for doc_name in tqdm(data, desc="文档", position=1, leave=False, ncols=60):
            ...
            for s in tqdm(range(len(samples)), desc="样本", position=2, leave=False, ncols=60):
                ...
                for j in range(repeated_n):
                    # 使用模型预测当前sample
                    if model_name == "GPT2":
                        pred = gpt2_pred(model, prompt)
                    elif model_name == "ChatGLM-6B":
                        pred = chatglm6b_create(model, prompt)
                ...
                result_index += 1
    # 保存结果
    output_file = f"{root_path}/Results/{model_name}/{model_name}_gold_mentions_{n_examples}-shots_{prefixes_source}_prefix_{repeated_n}-repeats-x.csv"
...
# 2. Generate Prompt
if 1:
    # 2.1 Define Parameters for Prefix
    if 1:
        n_examples = 2  # n-shot。就是从下边的simple_examples中选前n个用于生成prefix。也就是prefix中包含n个example，也就是n-shot。
        answer_choices = ["No", "Yes"]
    ...
    if 1:
        ...
        if 1:
            ...
            """
        # 2.2.2. Create prefix from SuperGLUE
        # if 1:
        ...
        prefixes_super_glue = []
        # 2.2.3. Create prefix from ECB+ train
        if 1:
...
"""data = {"2_1ecbplus.xml": dev["2_1ecbplus.xml"]}"""
#
"""prefix_types = ["simple","superglue", "ecb"]"""
...
annotate(data=data, templates=templates, prefixes=prefixes_ecb, prefixes_source="ecb", repeated_n=5, model_name="GPT2")
#
log_file.close()
